...
/** @type {{key:string,label:string,labelEn:string,zh?:RegExp,ar?:string[],en?:RegExp,arSet?:Set<string>}[]} */
const TOPICS = [
  {
    key: 'greeting',
    label: 'التحيّة والتعارف',
    labelEn: 'Greetings and introductions',
  ...
  },
  {
    ...
    labelEn: 'Travel and getting around',
  ...
  },
  {
    ...
    labelEn: 'People and relationships',
  ...
  },
  {
    ...
    labelEn: 'Speaking and communicating',
  ...
  },
  {
    ...
    labelEn: 'Feelings and qualities',
  ...
  },
  {
    ...
    labelEn: 'Describing and comparing',
...
// [4.5] The same fallback labels in English. Unit titles were Arabic-only —
// 191 of 191 — so an English learner navigated a curriculum whose every
// signpost was in a script they may not read. Added beside the Arabic rather
// than replacing it: both languages live at the call site in this codebase.
const POS_LABEL_EN = {
  noun: 'Nouns and things',
  verb: 'Everyday verbs',
  adjective: 'Adjectives and description',
  adverb: 'Adverbs and emphasis',
  pronoun: 'Pronouns and demonstratives',
  numeral: 'Numbers and counting',
  measure: 'Measure words',
  particle: 'Particles and function words',
  conjunction: 'Connectors',
  preposition: 'Prepositions',
  modal: 'Ability and desire verbs',
  fixed: 'Set expressions',
}

const DEFAULT_LABEL_EN = 'Words of the lesson'
...
function rankedLabels(words, threshold = 0.35) {
  return rankedTopics(words, threshold).map((t) => t.label)
}

/** The same ranking, returning whole topic records so a caller can read `labelEn`. */
function rankedTopics(words, threshold = 0.35) {
  const count = new Map()
...
/** The English twin of `posLabelFor`. */
function posLabelEnFor(words) {
  const count = new Map()
  for (const w of words) count.set(w.pos, (count.get(w.pos) || 0) + 1)
  let best = null
  let bestN = 0
  ...
  return POS_LABEL_EN[best] || DEFAULT_LABEL_EN
}
