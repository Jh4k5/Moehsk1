...
export default function Home() {
  ...
  return (
    <div className="min-h-screen flex flex-col" dir="rtl">
      {/* ─── Header ──────────────────────────────────────── */}
      <header className="sticky top-0 z-50 bg-white/95 dark:bg-[#0B1629]/95 backdrop-blur-sm border-b border-gray-100 dark:border-white/10 shadow-sm">
        <div className="px-4 py-3 flex items-center justify-between max-w-[1600px] mx-auto w-full">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-gray-900 dark:text-white">جِسر — تعلم الصينية HSK 1</h1>
              <p className="text-xs text-gray-500 dark:text-gray-400">{stats.total} كلمة • 26 قاعدة • مستوى مبتدئ</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <ThemeToggle />
            <Badge className="hidden sm:flex gap-1 bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 border-amber-200 dark:border-amber-800">
              <Flame className="w-3 h-3 text-amber-500" />
              <span>{store.dailyStreak} يوم متتالي</span>
            </Badge>
            <Badge className="hidden sm:flex gap-1 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 border-blue-200 dark:border-blue-800">
      ...
      <div className="flex-1 flex w-full max-w-[1600px] mx-auto">
        ...
        <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 dark:bg-[#0B1629]/95 backdrop-blur-sm border-t border-gray-200 dark:border-white/10 flex justify-around py-2 px-1 safe-area-bottom">
          {[
        ...
        <button onClick={() => setSidebarOpen(true)}
          className="lg:hidden fixed top-4 right-4 z-50 bg-white/90 dark:bg-[#0B1629]/90 backdrop-blur-sm p-2.5 rounded-xl shadow-lg border border-gray-100 dark:border-white/10 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
        ...
        {/* ─── Main Content ──────────────────────────────── */}
        <main className="flex-1 min-h-screen overflow-y-auto w-full p-4 md:p-6 pb-24 lg:pb-6 animate-fade-in" key={currentSection}>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 dark:text-gray-400 mt-1">مرحباً بك في رحلة تعلم اللغة الصينية!</p>
      </div>
      ...
      {/* Science Learning Tip of the Day */}
      {(() => {
        const scienceTips = [
          { icon: "🧠", tip: "تعلّمت كلمة جديدة؟ راجعها بعد ساعة، ثم يوم، ثم أسبوع — هذا نظام Ebbinghaus للذاكرة", title: "تكرار متباعد" },
          { icon: "🎯", tip: "الاسترجاع النشط أقوى من القراءة — اختبر نفسك بدل مجرد مشاهدة الكلمات", title: "استرجاع نشط" },
          { icon: "⏰", tip: "10 دقائق مراجعة يومية أفضل من ساعة دراسة أسبوعية — الانتظام يبني الذاكرة", title: "انتظام يومي" },
          { icon: "🗣️", tip: "انطق الكلمات بصوت عالٍ — الصوت يفعّل مناطق إضافية في الدماغ", title: "نطق بصوت عالٍ" },
          { icon: "📖", tip: "تعلّم الكلمات في جمل وليس منفردة — السياق يضاعف الحفظ", title: "تعلم في سياق" },
          { icon: "😴", tip: "النوم يرسّخ الذاكرة — راجع قبل النوم مباشرة", title: "مراجعة قبل النوم" },
          { icon: "🔄", tip: "الصعوبة المناسبة: تفهم 95% وكلمة واحدة جديدة — هذا معيار i+1", title: "مستوى مناسب" },
        ]
        const todayTip = scienceTips[new Date().getDay() % scienceTips.length]
        return (
          <Card className="border-0 shadow-sm bg-gradient-to-l from-blue-50/50 to-amber-50/30 dark:from-blue-900/20 dark:to-amber-900/10">
            <CardContent className="p-4">
              <div className="flex items-start gap-3">
                <span className="text-3xl">{todayTip.icon}</span>
                <div className="flex-1">
                  <div className="text-xs font-bold text-amber-600 dark:text-amber-400 mb-1">💡 نصيحة اليوم — {todayTip.title}</div>
                  <p className="text-sm text-gray-600 dark:text-gray-300 leading-relaxed">{todayTip.tip}</p>
                </div>
              </div>
            </CardContent>
          </Card>
