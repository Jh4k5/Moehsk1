...
export default function HanziSection() {
  ...
  const createWriter = useCallback((char: string) => {
    ...
    writerRef.current = HW.create(containerRef.current, char, {
      width: containerRef.current ? Math.min(300, containerRef.current.clientWidth) : 300,
      height: containerRef.current ? Math.min(300, containerRef.current.clientWidth) : 300,
      padding: 5,
      showOutline: true,
  ...
  return (
    <div dir="rtl" className="space-y-6">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-muted-foreground">تعلّم كتابة الحروف الصينية خطوة بخطوة</p>
        <Badge variant="outline" className="text-sm">
          {HSK1_CHARS.length} حرف من HSK1
        </Badge>
      </div>

      {/* Main Layout: Drawing Area (TOP on mobile, RIGHT on desktop) + Grid (BOTTOM on mobile, LEFT on desktop) */}
      <div className="flex flex-col lg:flex-row gap-6">
        {/* LEFT: Character Grid (appears below on mobile, left on desktop) */}
        <div className="order-2 lg:order-1 w-full lg:flex-[7] xl:flex-[8]">
          <Card className="border-red-200">
            <CardContent className="p-4">
              <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 xl:grid-cols-8 gap-2 max-h-[520px] overflow-y-auto">
                    ...
                    <motion.button
                      ...
                      className={
                        ...
                        (isSelected
                          ...
                          : 'bg-card border hover:border-red-300 hover:bg-red-50/50')
                      }
                    >
                      <span className={'font-chinese-serif text-2xl ' + (isSelected ? 'text-red-700' : 'text-foreground')}>
                        {char}
                      </span>
                      {info && (
                        <span className="text-[9px] text-muted-foreground mt-0.5 font-chinese-sans leading-tight">
                          {info.pinyin}
                        </span>
                      )}
                      {isSelected && (
        ...
        {/* RIGHT: Drawing Area + Controls (appears first on mobile, right on desktop) */}
        <div className="order-1 lg:order-2 w-full lg:flex-[5] xl:flex-[4] space-y-4">
          {/* Character Info Card */}
            ...
            >
              <Card className="border-red-200 bg-gradient-to-br from-red-50 to-white">
                <CardContent className="p-4 flex items-center gap-4">
                  <div className="flex flex-col items-center justify-center w-20 h-20 bg-card rounded-xl border-2 border-red-200 shadow-sm">
                    <span className="font-chinese-serif text-4xl text-red-700">{selectedChar}</span>
                  </div>
                  <div className="flex-1 space-y-1">
                    <div className="flex items-center gap-2">
                      <span className="font-chinese-sans text-xl font-bold text-foreground">
                        {charInfo?.pinyin || ''}
                      </span>
                      <button
                    ...
                    <p className="text-muted-foreground text-lg">{charInfo?.meaning || ''}</p>
                  </div>
                </CardContent>
              </Card>
            </motion.div>
          ...
          <Card className="border-red-200 overflow-hidden">
            <CardContent className="p-4 flex flex-col items-center">
              <div
                ref={containerRef}
                className="w-full max-w-[240px] sm:max-w-[280px] lg:max-w-[300px] aspect-square bg-card rounded-xl border-2 border-red-100 flex items-center justify-center relative mx-auto"
              >
                {!hanziWriterLoaded && (
